...
function computeCSS (element) {
  ...
  for (let rule of rules) {
    ...
    console.log('computed css', j, selectorParts)
  ...
  if (token.type === 'startTag') {
    ...
    if (top.tagName !== token.tagName) {
      ...
      if (top.tagName === 'style') {
        ...
        let c = ''
        if (top.children[0].content) {
          c = top.children[0].content
        }
        addCSSRules(c)
      }
      layout(top)
...
function tagName (c) {
  // console.log('doubleQuoteAttributeValue', currentAttribute)
  if (c.match(/^[\t\n\f ]$/)) {
...
function beforeAttributeValue (c) {
  // console.log('beforeAttributeValue', currentAttribute)
  if (c.match(/^[\t\n\f ]$/) || c === '/' || c === '>' || c === EOF) {
    return afterAttributeName(c)
  } else if (c === '\"') {
...
function afterQuoteAttributeValue (c) {
  // console.log('afterQuoteAttributeValue', c)
  if (c.match(/^[\t\n\f ]$/)) {
    return beforeAttributeName
  } else if (c === '/') {
    return selfClosingSatrtTag
